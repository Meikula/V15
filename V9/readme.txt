...
2:再该目录中的index.html上添加对图片的显示
3:启动服务端请求该页面,查看图片是否被正确加载

测试后发现图片不能正常显示
原因:
当浏览器请求页面后(一问一答完成)在解析页面时发现页面上需要加载其他的资源
时(比如index.html页面上要显示一张图片).浏览器会按照该资源指定的路径再次
发起请求去下载这个资源来进行显示.由于我们的服务端目前只接受一次请求后就关闭了
所以导致浏览器再次进行连接服务端失败,从而无法下载并显示这张图片.

由于我们已经完成了一问一答的流程,因此我们可以再WebSever类的start方法中将
接受客户端连接并启动线程处理的工作改为在死循环中进行了,这样一来服务端
就可以重复接收请求了.

完成上述工作后,再做另一个工作:
从doc.canglaoshi.org网站上下载稻草问答或学子商场的静态页面素材,并
...
因此无论浏览器现在请求是么样的资源我们响应时都告知是text/html即"页面"
