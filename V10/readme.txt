上一个版本我们通过重构HttpResponse发送响应代码将响应头的发送改为根据设置的响应
...
2:Map:中内容的扩充(要支持所有的资源类型Content-Type的值,共1000多种)

解决Map的复用问题:
1:在com.webserver.http包下新建一个类:HttpContext
  这个类用于保存所有的Http协议规定的可以重用的内容
...
这样一来ClientHandler每次获取到资源后缀就无需创建Map,而是从HttpContext中调用getMimeType
方法直接根据后缀获取Content-Type的值设置响应头节课.

重构ClientHandler中设置响应头代码.
由于一个响应中只要有响应正文就应当包含两个响应头Content-Type和Content-Length,因此我们可以把
